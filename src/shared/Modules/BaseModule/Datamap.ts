...
// TODO: Consider:
// - Switching access methods to use longitude and latitude instead of coordinates
//   - Think about performance overhead from runtime unit conversion
// - Having `data` as raw height --> convert to altitude at runtime
//   - Probably store true altitude in data array
// - Caching access() calls in `data`

export default class Datamap extends BaseModule {
	constructor(
		readonly data: number[][],
		/** For data, in `[x, y]` */
		readonly dimensionSizes: [number, number],
	 ...
	 */
	access(x: number, y: number): number {
	...
	// nearestNeighborInterp(x: number, y: number): number {
	// 	return this.access(math.round(x), math.round(y));
	// }

	// bilinearInterp(x: number, y: number): number {
	...
	bicubicInterp(x: number, y: number): number {
		if (this.data[y]?.[x]) return this.data[y][x];
		else {
			const xFloor = math.floor(x);
			const xCeil = math.ceil(x);
			const yFloor = math.floor(y);
			...
			return this.data[y][x] = catmullRom(
				yPortion,
				xInterpRows[0],
				xInterpRows[1],
				xInterpRows[2],
	...
	longLatToCoords(longitude: number, latitude: number): [number, number] {
        const x = (longitude / (2 * math.pi) + 0.5) * this.dimensionSizes[0];
        const y = (latitude / math.pi - 0.5) * this.dimensionSizes[1];

        return [x, y];
    }

	// /** Longitude and latitude in radians */
    // coordsToLongLat(x: number, y: number): [number, number] {
    //     const longitude = (x / this.dimensionSizes[0] - 0.5) * 2 * math.pi;
    //     const latitude = (0.5 - y / this.dimensionSizes[1]) * math.pi;

    //     return [longitude, latitude];
    // }

	convertHeight(raw: number): number {
		return raw / this.maxHeight;
	}

	// groundColor(longitude: number, latitude: number): Color3 {
	// 	// TODO
	// }
}
